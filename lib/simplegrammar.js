...
function NotCharacterRule(ch) {
    ...
    this.processSource = function (source) {
        ...
        for (var next = source.next(); next && next.length && next.length == 1; next = source.next())
            if (next == ch)
                break;
            else
                result += next;
...
util.inherits(NotCharacterRule, Rule);
...
function createRule(param) {
    ...
    if (typeof param === 'string' && param.length === 2 && param[0] == '~')
        return new NotCharacterRule(param[1]);
